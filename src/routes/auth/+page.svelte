<script>
	...
	const checkAgreeTerms = () => {
		if (!agreedTerms) {
			toast.error(
				'You must agree to the Terms of Service, Privacy Policy, and Cookie Policy to proceed.'
			);
			return false;
		}
		return true;
						...
						>
							...
							<div class="mt-5">
								{#if $config?.features.enable_login_form || $config?.features.enable_ldap}
									{#if mode === 'ldap'}
										<button
											class="bg-gray-700/5 hover:bg-gray-700/10 dark:bg-gray-750 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											type="submit"
										>
											{$i18n.t('Authenticate')}
										</button>
