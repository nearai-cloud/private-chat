...
async def process_web_search(
    ...
    try:
        logging.debug(f"web search for query: {form_data.query}")
    ...
    try:
        # Use web search results directly, no need to fetch web pages
        urls = [result.link for result in web_results]

        # Create documents from search results directly
        docs = []
        ...
        log.debug(
            f"Created {len(docs)} documents from web search results without web fetching"
        )
