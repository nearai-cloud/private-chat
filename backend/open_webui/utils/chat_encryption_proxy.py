...
from open_webui.models.chats import (ChatForm, ChatImportForm, ChatModel,
                                     ChatTable)
...
class ChatTableEncryptionProxy(ChatTable):
    ...
    def __init__(self):
        ...
        env_hint = "" if self._encryption_enabled else " (set ENABLE_CHAT_ENCRYPTION=true to enable)"
        log.info(f"ChatTableEncryptionProxy initialized with per-user encryption {log_level}{env_hint}")
    ...
    def _store_encrypted_chat(self, chat_record, chat_data: dict, user_id: str) -> None:
        ...
        if self._encryption_enabled:
            try:
                ...
                log.error(f"Encryption failed, falling back to unencrypted storage: {e}")
    ...
    def import_chat(self, user_id: str, form_data: ChatImportForm) -> Optional[ChatModel]:
        """Override to encrypt chat data before importing."""
        if not self._encryption_enabled:
            return super().import_chat(user_id, form_data)
        
        # For simplicity, delegate to parent method for now
        # TODO: Implement proper encrypted import
        return super().import_chat(user_id, form_data)
    ...
    def upsert_message_to_chat_by_id_and_message_id(
        ...
        if not self._encryption_enabled:
            return super().upsert_message_to_chat_by_id_and_message_id(id, message_id, message)
    ...
    def _decrypt_chat_models_list(self, chat_models: list[ChatModel]) -> list[ChatModel]:
    ...
    def get_chat_list_by_user_id(
        ...
        """Override to decrypt chat data after retrieving."""
        results = super().get_chat_list_by_user_id(user_id, include_archived, filter, skip, limit)
    ...
    def get_archived_chat_list_by_user_id(
        ...
        """Override to decrypt chat data after retrieving."""
        results = super().get_archived_chat_list_by_user_id(user_id, filter, skip, limit)
        return self._decrypt_chat_models_list(results) if self._encryption_enabled else results
    ...
    def get_chats_by_user_id_and_search_text(
        ...
        """Override to decrypt chat data after retrieving."""
        results = super().get_chats_by_user_id_and_search_text(
    ...
    def insert_shared_chat_by_chat_id(self, chat_id: str) -> Optional[ChatModel]:
        ...
        result = super().insert_shared_chat_by_chat_id(chat_id)
        if result and isinstance(result.chat, str) and self._encryption_enabled:
            try:
                result.chat = decrypt_chat_data(result.chat, result.user_id)
            except Exception as e:
                log.error(f"Failed to decrypt shared chat {chat_id}: {e}")
                return None
        return result
    
    def update_shared_chat_by_chat_id(self, chat_id: str) -> Optional[ChatModel]:
        """Override to handle shared chat encryption."""
        result = super().update_shared_chat_by_chat_id(chat_id)
        if result and isinstance(result.chat, str) and self._encryption_enabled:
            try:
                result.chat = decrypt_chat_data(result.chat, result.user_id)
            except Exception as e:
                log.error(f"Failed to decrypt updated shared chat {chat_id}: {e}")
                return None
        return result
