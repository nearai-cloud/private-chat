...
class UserDataEncryptionService:
    ...
    def __init__(self):
        self.key_service_base_url = os.getenv(
            'KEY_SERVICE_BASE_URL'
        )
    ...
    def _get_user_encryption_key(self, user_id: str) -> bytes:
        ...
        if existing_key:
            # User has an existing key, decrypt and cache it
            decrypted_key_hex = self._decrypt_data_key(existing_key.encrypted_data_key)
            ...
            # User doesn't have a key, generate a new one
    ...
    def decrypt_data(self, encrypted_data: str, user_id: str) -> dict:
        ...
        # Check if data is plain JSON (backward compatibility)
        if self._is_plain_json(encrypted_data):
            try:
    ...
    def _is_plain_json(self, data: str) -> bool:
        """Check if data appears to be plain JSON (not encrypted)."""
        if not data:
            return False
        stripped = data.strip()
