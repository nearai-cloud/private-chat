...
def test_otlp_connection(exporter):
    ...
    try:
        ...
        logger.info("Connection test completed (note: actual connectivity will be verified when real spans are exported)")
        return True
...
def setup(app: FastAPI, db_engine: Engine):
    ...
    try:
        ...
        logger.info(f"OTLP Exporter created with endpoint: https://otlp.nr-data.net:4317")
