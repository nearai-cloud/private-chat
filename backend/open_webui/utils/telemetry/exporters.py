...
class LazyBatchSpanProcessor(BatchSpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        logger.debug(f"Processing span: {span.name} (trace_id: {span.get_span_context().trace_id:032x})")
    ...
    def worker(self):
        ...
        def logged_export(spans):
            ...
            for span in spans:
                logger.debug(f"Exporting span: {span.name} (trace_id: {span.get_span_context().trace_id:032x})")
